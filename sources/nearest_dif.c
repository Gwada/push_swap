...
{
//	ft_printf("{bold}{underline}{yellow}IN\tNEXT DIF\tALNBR = %d{eoc}\n", ALNBR);//
	int			i;
	int			fst;
	int			lst;
	int			n_dep_fst;
...
//	ft_printf("{green}{bold}best dif dep = %d\n{eoc}", *best_dif_dep);

	r->b_rot = (r->nb_a - (r->nb_a - fst) <= r->nb_a - lst) ? fst : lst;
//	ft_printf("{bold}{underline}{yellow}END\tNEXT DIF{eoc}\n\n");//
}
